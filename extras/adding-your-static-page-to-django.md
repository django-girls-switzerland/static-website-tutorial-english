...
{% raw %}Add the `{% extends 'blog/base.html' %}` and `{% block content %}` and `{% endblock content %}`, templatetags to your about.html page. The 'extends' tag should come at the very top and the block tags should surround your actual page content.{% endraw %}
